...
$$D_\text{KL}(q(\theta),p(\theta | \mathbf{Y}))  = \log \ell(\mathbf{Y}) - \left[ \mathbf{E}_q[\log \ell(\mathbf{Y}|\theta)\pi(\theta)] -\mathbf{E}_q[\log q(\theta)]\right] $$ 
  - $\log \ell(\mathbf{Y})$ independent of $q$ 
  - Minimizing the KL is equivalent to  maximizing the ELBO
    
  $$\text{ELBO}(q) = \mathbf{E}_q[\log \ell(\mathbf{Y}|\theta)\pi(\theta)] -\mathbf{E}_q[\log q(\theta)]$$
$$\text{ELBO}(q) = \mathbf{E}_q[\log \ell(\mathbf{Y}|\theta)] -\mathbf{E}_q\left[\log \frac{q(\theta)}{\pi(\theta)}\right] = \mathbf{E}_q[\log \ell(\mathbf{Y}|\theta)] - D_{\text{KL}}(q,\pi)$$
---
